...
class slu_data():
    def __init__(self):
        ...
        valid_talker = open('../Data/valid/talker', 'r')
        ...
        self.train_talkers = self.get_talker_labels(train_talker)
        self.valid_talkers = self.get_talker_labels(valid_talker)
        self.test_talkers = self.get_talker_labels(test_talker)
        ...
        glove = open('PATH_TO_GLOVE/glove.6B.200d.txt', 'r')
    ...
    def get_talker_labels(self, data_file):
        who = list()
        data_file.seek(0)
        for line in data_file:
            talker = line.strip('\n')
    ...
    def get_train_batch(self, batch_size, role=None):
        ...
        for batch_idx in batch_indices:
            ...
            talker = self.train_talkers[batch_idx]
            ret_talker_batch.append(talker)
        return ret_nl_batch, ret_intent_batch, ret_dist_batch, ret_talker_batch
    ...
    def get_valid_batch(self, batch_size):
        ...
        for batch_idx in batch_indices:
            ...
            ret_nl_batch.append(nl_sentences)
            ret_intent_batch.append(intent)
            talker = self.valid_talkers[batch_idx]
            ret_talker_batch.append(talker)
        return ret_nl_batch, ret_intent_batch, ret_talker_batch
    ...
    def get_test_batch(self):
        ...
        for batch_idx in batch_indices:
            ...
            ret_dist_batch.append(dist)
            talker = self.test_talkers[batch_idx]
            ret_talker_batch.append(talker)
        return ret_nl_batch, ret_intent_batch, ret_dist_batch, ret_talker_batch
    ...
    def read_GloVe(self, glove):
        ...
        id2word[word2id[word]]="UNKnown"
        embedding_matrix.append([np.random.uniform(-0.1,0.1) for _ in range(200)]) #when 200 dim.
        print("Done. %d words loaded from GloVe!"%len(embedding_matrix))
