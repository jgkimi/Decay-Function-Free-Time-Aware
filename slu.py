...
parser.add_argument('--use_distinct_w', default='no', choices=['yes', 'no'], help='yes: distinct / no: shared')
parser.add_argument('--talker_applied_to', default=['Dist'], help='This should select elements among the list att_to. If empty, will not be used.')
parser.add_argument('--att_to', default=['Dist'], help='This should be a list which consists of Intent & Dist.')
parser.add_argument('--att_out', default=['Dist', 'Intent'], help='This should be a list which consists of Intent & Dist.')
...
def process_batch(batch_nl, batch_intent, max_seq_len, intent_pad_id, nl_pad_id, total_intent, batch_distance, batch_talker):
    nl_pad_id += 1
    hist_len = 7#the context length for each speaker
    current_nl = list()
    current_nl_len = list()
    current_talker = list()
    ground_truth = list()
    ...
    for i in batch_distance:
        #i_prime = [49 if x>=49 else x for x in i]
        #history_distance.append(i_prime[:-1])
        history_distance.append(i[:-1])

    for i in batch_talker:
        current_talker.append(i)

    return history_nl, history_nl_len, current_nl, current_nl_len, history_intent, ground_truth, history_distance, current_talker
...
if __name__ == '__main__':
    ...
    sess.run(model.init_embedding, feed_dict={model.read_embedding_matrix:data.embedding_matrix})#model embedding_matrix: 400001 while data embedding_matrix: 400000
    test_f1_scores = list()
    ...
    for cur_epoch in range(epoch):
        ...
        for cnt in range(50):
            ...
            history_nl, history_nl_len, current_nl, current_nl_len, history_intent, ground_truth, history_distance, current_talker = \
            process_batch(batch_nl, batch_intent, max_seq_len, total_intent-1, total_word-1, total_intent, batch_distance, batch_talker)
        ...
        f1sc = f1_score(test_label, test_pred, average="binary")
        print("testing f1 score: %f"%f1sc)
        test_f1_scores.append(f1sc)
        
    print("max test f1 score:  %f"%max(test_f1_scores))
